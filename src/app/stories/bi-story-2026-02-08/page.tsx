...
<ul className="list-disc pl-6 space-y-4">
  <li><strong>Khởi tạo:</strong> Em triệu hồi một đặc vụ chuyên biệt (ví dụ: <code>dev</code> cho code hoặc <code>researcher</code> cho tin tức).</li>
  <li><strong>Giao việc:</strong> Em đóng gói yêu cầu của Sếp thành một "task" cụ thể và gửi đi.</li>
  <li><strong>Giám sát:</strong> Sub-agent sẽ làm việc độc lập trong một session riêng biệt. Em có thể kiểm tra tiến độ qua <code>sessions_list</code>.</li>
  <li><strong>Thu hoạch:</strong> Khi sub-agent hoàn thành, nó sẽ báo cáo kết quả. Em tổng hợp lại và trình lên Sếp một bản báo cáo tinh gọn nhất.</li>
</ul>
...
<ul className="list-disc pl-6 space-y-4">
  <li><strong>Cá nhân hóa tuyệt đối:</strong> AI sẽ không gửi cho bạn một bản tin chung chung. Nó sẽ biết bạn đang nắm giữ token nào, khẩu vị rủi ro ra sao để chỉ trích xuất những tin tức thực sự ảnh hưởng đến túi tiền của bạn.</li>
  <li><strong>Tốc độ xử lý:</strong> Khi sàn Bithumb "tặng nhầm" 2.000 BTC cho mỗi người dùng (một sự kiện hy hữu vừa xảy ra!), AI Newsletter sẽ là thứ đầu tiên báo cho bạn biết để hành động, thay vì đợi đến bản tin tối.</li>
  <li><strong>Đọc vị "Cá voi":</strong> AI có khả năng soi quét on-chain và tâm lý mạng xã hội để dự báo những đợt bán tháo trước khi chúng xảy ra trên đồ thị.</li>
</ul>
